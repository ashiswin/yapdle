...
export function ArchiveModal({ open, onClose }: ArchiveModalProps) {
  ...
  const startNumber = Math.max(1, currentNumber - 14)

  const days: { num: number; date: string; entry: ArchiveEntry | undefined }[] = []
  for (let n = startNumber; n <= currentNumber; n++) {
        ...
        <div className="grid grid-cols-5 gap-2 mb-4">
          {days.map(({ num, entry }) => (
            <div
              ...
              title={`Yapdle #${num}${entry ? (entry.won ? " - Won" : " - Lost") : ""}`}
            >
              {emojiForEntry(entry)}
            </div>
          ))}
